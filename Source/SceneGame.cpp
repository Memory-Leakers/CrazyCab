...
bool SceneGame::InitScene()
{
	...
	for (int i = 0; i < _app->map->mapObjects.count(); i++)
	{
		...
		g->pBody->SetPos(_app->map->mapObjects[i].position.x + ((_app->map->mapObjects[i].dimensions[0] / 2) * 16), 1, _app->map->mapObjects[i].position.y + ((_app->map->mapObjects[i].dimensions[0] / 2)*16));
